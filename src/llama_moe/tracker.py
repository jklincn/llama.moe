...
class MetricsTracker(threading.Thread):
    ...
    def check_idle(self):
        try:
            slots_url = f"http://{self.host}:{self.port}/slots"
            headers = {"Authorization": f"Bearer {self.api_key}"}
            resp = requests.get(slots_url, headers=headers, timeout=0.5)
            if resp.status_code == 200:
                slots = resp.json()
                # Check if any slot is not idle (state != 0)
                for slot in slots:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if total_tokens >= self.threshold:
                        logger.info(
                            f"Threshold reached: {total_tokens} >= {self.threshold}. Waiting for idle..."
                        )
